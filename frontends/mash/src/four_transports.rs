#![forbid(unsafe_code)]
//! The four avenues this web server can use to reach `happy-wakey-api-server`.
//!
...
//! to happy-wakey — the environment prefix, the service slug that names its NATS
//! subjects, and the api route that accepts an envelope.
//!
//! | # | Mode | What happens |
//! |---|------|--------------|
//! | 1 | `direct_read` | this server queries Postgres itself through `happy-wakey-lib-core`, read-only |
//! | 2 | `http` | stateless request to the `happy-wakey-api-server` cluster |
//! | 3 | `tcp` | framed, held-open connection to the `happy-wakey-api-server` cluster |
...
//! `happy-wakey-lib-core`'s read context, and hand it to [`gateway_from_env`]. See
...
///
/// `HAPPY_WAKEY_API_URL`, `HAPPY_WAKEY_NATS_URL`, and the rest; see the
/// table in `ores-transport`'s README.
pub const ENV_PREFIX: &str = "HAPPY_WAKEY";
...
pub const SERVICE_SLUG: &str = "happy-wakey";
