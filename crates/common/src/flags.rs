...
#[cfg(not(test))]
pub fn process_control(binary_name: &str, version: &str) -> Result<Option<String>, String> {
    let values = try_resolved()?;
    if values.get("CLI_HELP_REQUESTED").map(String::as_str) == Some("true") {
        return help_table(binary_name).map(Some);
    }
    if values.get("CLI_VERSION_REQUESTED").map(String::as_str) == Some("true") {
        return Ok(Some(format!("{binary_name} {version}\n")));
    }
    Ok(None)
}
...
pub fn process_control(_binary_name: &str, _version: &str) -> Result<Option<String>, String> {
    Ok(None)
}
...
fn help_table(binary_name: &str) -> Result<String, String> {
    ...
    let mut output = format!("Usage: {binary_name} [OPTIONS]\n\nOptions:\n");
...
mod tests {
    ...
    fn help_is_derived_from_the_audited_contract() {
        let help = help_table("happy-wakey-mash").expect("help");
        assert!(help.contains("--help"));
        assert!(help.contains("Options:"));
        assert!(help.starts_with("Usage: happy-wakey-mash"));
    }
}
