# The four avenues between `happy-wakey-mash` and `happy-wakey-api-server`

`happy-wakey` follows the fleet-wide contract in
...
|---|------|------|-------------------|
| 1 | `direct_read` | `happy-wakey-mash` → Postgres, via `happy-wakey-lib-core` | a read on the page's critical path |
| 2 | `http` | `happy-wakey-mash` → `happy-wakey-api-server`, stateless | the default, and anything that writes |
| 3 | `tcp` | `happy-wakey-mash` ⇄ `happy-wakey-api-server`, held open | chatty internal calls where the handshake dominates |
| 4 | `jet_stream` | `happy-wakey-mash` → NATS → `happy-wakey-api-server` | work the caller does not need to watch finish |

All four carry the same envelope and land on the same handler, so a
`happy-wakey-e2e` suite can drive one operation four ways and assert the answers
...
|---|---|
| Slug | `happy-wakey` |
| Environment prefix | `HAPPY_WAKEY_` |
| Request subject | `happy-wakey.api.operation.v1` |
| Result subject | `happy-wakey.api.operation-result.v1` |
| Durable consumer | `happy-wakey-api-server-v1` |
...
|----------|--------|--------------|
| `HAPPY_WAKEY_READONLY_DATABASE_URL` | 1 | no direct reads |
| `HAPPY_WAKEY_API_URL` | 2 | no stateless HTTP |
| `HAPPY_WAKEY_API_MTLS_ADDR` | 3 | no stateful TCP |
| `HAPPY_WAKEY_API_TLS_SERVER_NAME` | 3 | plaintext to a TLS-terminating mesh |
| `HAPPY_WAKEY_WEB_CLIENT_CERT_FILE` | 3 | ” |
| `HAPPY_WAKEY_WEB_CLIENT_KEY_FILE` | 3 | ” |
| `HAPPY_WAKEY_API_CA_FILE` | 3 | ” |
| `HAPPY_WAKEY_NATS_URL` | 4 | no asynchronous avenue |

A missing variable disables its avenue. A variable that is present and unsafe
fails startup: a plaintext `HAPPY_WAKEY_API_URL` off loopback, a
`HAPPY_WAKEY_NATS_URL` that is not `tls://`, or mutual TLS with two of its
four files set.

In-cluster, `HAPPY_WAKEY_NATS_URL` points at the messaging service defined in
...
**In `happy-wakey-mash`** — implement `DirectReader` over `happy-wakey-lib-core`:
...
impl DirectReader<Operation, Projection> for LibCoreReader {
        ...
        -> Result<Projection, TransportError>
    {
        ...
        // not an access check.
        // Then build the query with happy-wakey-lib-core's query builders.
    }
...
**In `happy-wakey-api-server`** — implement `OperationHandler` once, and hand the same
...
**Avenue 1 is read-only twice over.** `happy-wakey-lib-core` gates its write surface
behind a `read-write` cargo feature and migrations behind `migrate`. Depend on
it from `happy-wakey-mash` as:

```toml
happy-wakey-lib-core = { git = "…", default-features = false, features = ["read-only"] }
```

so a mutating call is a *compile error*, not a code-review miss. Separately,
`HAPPY_WAKEY_READONLY_DATABASE_URL` must name a Postgres role without
`INSERT`, `UPDATE`, `DELETE` or DDL. A mistake has to defeat both. Migrations
belong to `happy-wakey-lib-core` and `declarative-migrations`, never to a web server.
